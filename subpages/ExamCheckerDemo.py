    ...
    try:
        ...
        if token and key and chat_input and (len(variables) == len(variable_dict)):
            ...
            with st.chat_message("assistant"):
                try:
                    response, sources, trace_id = generate_response(api_token = token, key_input = key, conv_memory= conv_memory , variable_dict = variable_dict, context_input = None, file_id = file_id)
